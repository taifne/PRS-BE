...
export class PunchService {
  ...
  async generatePunchesForUser(
    ...
    while (currentDate <= endDate) {
      if (!isWeekend(currentDate)) {
        ...
        let punch = await this.punchModel.findOne({
          user: userId,
          date: dateOnly,
        });
        ...
        if (!punch) {
          punch = new this.punchModel({
            ...
            lastPunchOut: createDateWithTime(
              dateOnly,
              17,
              randomMinuteOffset(),
            ),
          ...
          punch.lastPunchOut = createDateWithTime(
            dateOnly,
            17,
            randomMinuteOffset(),
          );
  ...
  async getTotalWorkingHoursForMonth(
    userId: string,
    month: string,
  ): Promise<number> {
    ...
    for (const punch of punches) {
      if (punch.firstPunchIn && punch.lastPunchOut) {
        const duration =
          punch.lastPunchOut.getTime() - punch.firstPunchIn.getTime();
        if (duration > 0) {
