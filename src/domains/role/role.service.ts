...
export class RoleService extends BaseService<RoleDocument> {
  ...
  // Optional wrapper for clarity
  async createRole(dto: CreateRoleDto) {
    // Use enhanced BaseService.create to check by 'name' and throw if exists
    return this.create(dto, 'name', true);
  }
  ...
  async updateMenusInRole(roleId: string, menuIds: string[]): Promise<Role> {
    ...
    const menuObjectIds = menuIds.map((id) => {
      if (!Types.ObjectId.isValid(id))
        throw new BadRequestException(`Invalid menu ID: ${id}`);
